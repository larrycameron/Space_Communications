...
struct Cell {std::string Name,Base_Scenario,Factor_Name;double Factor_Value{},Signal_Factor{},Background{},PAT_Per_Hop{};bool Disable_Relay2{};};
...
Outcome Evaluate(const Packet& p,const Route& r,const std::string& stream){Outcome o;o.Optical_Uniform=Optical_Uniform(p.Seed,p.Index,stream);o.Latency=(static_cast<double>(PACKET_BITS)/DATA_RATE_BPS)*r.Hops+r.PAT+r.Propagation;o.Arrival=p.Release+o.Latency;if(p.Contact_Uniform>r.Contact)o.Contact_Failure=true;else if(o.Arrival>p.Deadline)o.Expired=true;else if(o.Optical_Uniform>r.Optical)o.Optical_Failure=true;else o.Delivered=true;o.Dropped=!o.Delivered&&!o.Expired;return o;}

std::vector<Cell> Cells(const std::string& family,bool include_null){
 if(family=="pat"){std::vector<Cell> x;for(double pat:{30.,60.,120.}){if(include_null){x.push_back({"PAT"+std::to_string((int)pat)+"__Control","Control","PAT_s_Per_Hop",pat,1.,4.,pat,false});x.push_back({"PAT"+std::to_string((int)pat)+"__Direct_Background_16","Direct_Background_16","PAT_s_Per_Hop",pat,1.,16.,pat,false});}x.push_back({"PAT"+std::to_string((int)pat)+"__Direct_Pointing_25pct_Power","Direct_Pointing_25pct_Power","PAT_s_Per_Hop",pat,.25,4.,pat,false});x.push_back({"PAT"+std::to_string((int)pat)+"__Direct_Combined_25pctPower_BG16","Direct_Combined_25pctPower_BG16","PAT_s_Per_Hop",pat,.25,16.,pat,false});}return x;}
 if(family=="no-relay2")return {{"NoRelay2__Direct_Pointing_25pct_Power","Direct_Pointing_25pct_Power","Relay2_Enabled",0.,.25,4.,60.,true},{"NoRelay2__Direct_Combined_25pctPower_BG16","Direct_Combined_25pctPower_BG16","Relay2_Enabled",0.,.25,16.,60.,true}};
 if(family=="pointing"){std::vector<Cell> x;for(double f:{1.,.75,.50,.25})x.push_back({"PointingFactor"+std::to_string(f),"Direct_Pointing_25pct_Power","Direct_Signal_Factor",f,f,4.,60.,false});return x;}
 throw std::runtime_error("Family must be pat, no-relay2, or pointing");}
...
int main(int argc,char** argv){if(!Tests())return 1;if(argc<5){std::cerr<<"Usage: Robustness_Experiment FAMILY TRIAL_CSV PACKET_CSV OUTPUT_DIR [WORLD_LIMIT] [--include-null]\n";return 2;}try{
 ...
 std::string prefix=family=="pat"?"PAT":family=="no-relay2"?"No_Relay2":"Pointing";std::ofstream route(outdir+"/"+prefix+"_Route_Scores.csv"),summary(outdir+"/"+prefix+"_Trial_Summary.csv"),manifest(outdir+"/"+prefix+"_Manifest.txt");if(!route||!summary||!manifest)throw std::runtime_error("Could not create outputs");route<<std::setprecision(17);summary<<std::setprecision(17);
 ...
 summary<<"Condition,Base_Scenario,Factor_Name,Factor_Value,Trial,Seed,Policy,Selected_Route,Generated,Delivered,Expired,Dropped,Contact_Failures,Optical_Failures,Reliability,Delivered_Bits,Mean_Delivered_Latency_s\n";std::map<Summary_Key,Summary> sums;bool accounting=true;for(const auto& p:packets)for(const auto& cell:cells)for(const auto& policy:POLICIES){const auto& r=states.at({p.Seed,cell.Name,policy});auto o=Evaluate(p,r,cell.Base_Scenario);accounting&=((int)o.Delivered+(int)o.Expired+(int)o.Dropped)==1;auto& s=sums[{p.Seed,cell.Name,policy}];s.Trial=p.Trial;s.Seed=p.Seed;s.Cell=cell.Name;s.Base=cell.Base_Scenario;s.Factor=cell.Factor_Name;s.Factor_Value=cell.Factor_Value;s.Policy=policy;s.Route=r.Name;++s.Generated;s.Delivered+=o.Delivered;s.Expired+=o.Expired;s.Dropped+=o.Dropped;s.Contact_Failures+=o.Contact_Failure;s.Optical_Failures+=o.Optical_Failure;if(o.Delivered){s.Bits+=PACKET_BITS;s.Latency_Sum+=o.Latency;}}
