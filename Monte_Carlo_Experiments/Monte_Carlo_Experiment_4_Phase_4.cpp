...
//
// Phase 4 diagnostic:
// Add deterministic/shared propagation-delay state while
// preserving the matched B1-B4 Monte Carlo world design.
//
...
struct RQ4_Shared_World
{
    ...
    // Phase 4 diagnostic Earth-Mars distance.
    // Generated from an independent seed substream so adding this
    // field does not alter the previously validated Phase 3 draws.
    ...
    const std::vector<RQ4_Candidate_Route>& routes)
{
    ...
    // --------------------------------------------------------
    // PHASE 4 DIAGNOSTIC GEOMETRY
    // --------------------------------------------------------
    // The three relays are temporarily treated as collinear
    // points along the same Earth-Mars line at 25%, 50%, and 75%.
    // This is NOT the final journal relay architecture.
    //
    // Under this diagnostic geometry, the sum of the two relay-hop
    // distances equals the direct Earth-Mars distance. That gives us
    // a clean test of propagation-delay calculation, hop aggregation,
    // CSV persistence, and matched-policy state transfer before final
    // orbital relay placement is introduced.
    // --------------------------------------------------------
    ...
    std::uint32_t seed)
{
    ...
    // Phase 4 uses a separate deterministic substream for distance.
    // This preserves every previously validated Phase 3 random draw.
    std::mt19937 distance_generator(
        seed ^ 0xA5A5A5A5u);

    std::uniform_real_distribution<double>
        Earth_Mars_Distance_Distribution(
            5.46e10,
            4.01e11);
