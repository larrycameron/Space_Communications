...
//
// Phase 11 post-pilot diagnostic:
// Preserve the frozen Phase 10 physics, B1-B4 scoring, matched worlds,
// packet mechanics, and common-random-number contact streams. Re-run only
// the five pilot seeds in which B3 selected Relay 0. For every available
// route, report the B3/B4 score components and counterfactual packet-level
// performance under the same realized world. No routing formula is changed
// in this file; it diagnoses the evidence needed for the next model revision.
//
...
// ============================================================
// PHASE 10 - FINAL EXPERIMENT-READINESS CERTIFICATION
// No new physics: certify the frozen Phase 9 engine before pilot use.
// ============================================================
static constexpr std::uint32_t PHASE_10_VALIDATION_SEED_LAST=49999u;
static constexpr std::uint32_t PHASE_10_PILOT_SEED_FIRST=100000u;
static constexpr std::uint32_t PHASE_10_PRODUCTION_SEED_FIRST=1000000u;

struct Phase10_Certification_Result { int Checks_Passed{}; int Checks_Total{}; bool Overall_Pass{true}; };

void Phase10_Record_Check(Phase10_Certification_Result& r,const std::string& name,bool pass)
{
    ++r.Checks_Total; if(pass) ++r.Checks_Passed; else r.Overall_Pass=false;
    std::cout<<"    "<<name<<": "<<(pass?"PASS":"FAIL")<<"\n";
}
bool Phase10_File_Exists_And_Nonempty(const std::string& p)
{
    std::error_code ec; return std::filesystem::exists(p,ec)&&!ec&&std::filesystem::file_size(p,ec)>0&&!ec;
}
bool Phase10_Seed_Plan_Is_Separated()
{
    return PHASE_10_VALIDATION_SEED_LAST<PHASE_10_PILOT_SEED_FIRST &&
           PHASE_10_PILOT_SEED_FIRST<PHASE_10_PRODUCTION_SEED_FIRST;
}
bool Phase10_Packet_Accounting_Is_Valid(const RQ4_Phase9_Trial_Metrics& m)
{
    return m.Generated_Packets==m.Delivered_Packets+m.Expired_Packets+m.Dropped_Packets &&
           m.Dropped_Packets==m.Contact_Failure_Drops+m.No_Route_Drops+
                              m.Route_Unavailable_Drops+m.Invalid_State_Drops;
}
bool Phase10_Metrics_Equivalent(const RQ4_Phase9_Trial_Metrics& a,const RQ4_Phase9_Trial_Metrics& b)
{
    const double t=1e-12;
    return a.Generated_Packets==b.Generated_Packets && a.Delivered_Packets==b.Delivered_Packets &&
    a.Expired_Packets==b.Expired_Packets && a.Dropped_Packets==b.Dropped_Packets &&
    a.Contact_Failure_Drops==b.Contact_Failure_Drops && a.No_Route_Drops==b.No_Route_Drops &&
    a.Route_Unavailable_Drops==b.Route_Unavailable_Drops && a.Invalid_State_Drops==b.Invalid_State_Drops &&
    std::abs(a.Reliability-b.Reliability)<=t && std::abs(a.Delivered_Bits-b.Delivered_Bits)<=t &&
    std::abs(a.Delivered_Throughput_Bps-b.Delivered_Throughput_Bps)<=t &&
    std::abs(a.Mean_Delivered_Latency_Seconds-b.Mean_Delivered_Latency_Seconds)<=t &&
    std::abs(a.Median_Delivered_Latency_Seconds-b.Median_Delivered_Latency_Seconds)<=t &&
    std::abs(a.P95_Delivered_Latency_Seconds-b.P95_Delivered_Latency_Seconds)<=t &&
    std::abs(a.Total_PAT_Burden_Seconds-b.Total_PAT_Burden_Seconds)<=t &&
    std::abs(a.Mean_PAT_Burden_Per_Generated_Packet_Seconds-b.Mean_PAT_Burden_Per_Generated_Packet_Seconds)<=t &&
    a.Route_Selection_Counts==b.Route_Selection_Counts &&
    a.No_Route_Selection_Count==b.No_Route_Selection_Count && a.Route_Switch_Count==b.Route_Switch_Count;
}
Phase10_Certification_Result Run_Phase10_Preflight_Certification()
{
    Phase10_Certification_Result r;
    std::cout<<"Phase 10 preflight experiment-readiness certification:\n";
    Phase10_Record_Check(r,"TEST 1 - validation/pilot/production seed ranges separated",Phase10_Seed_Plan_Is_Separated());
    Phase10_Record_Check(r,"TEST 2 - pilot and production seed bases distinct",PHASE_10_PILOT_SEED_FIRST!=PHASE_10_PRODUCTION_SEED_FIRST);
    Phase10_Record_Check(r,"TEST 3 - Phase 9 packet configuration retained",
        PHASE_9_PACKET_SIZE_BITS>0&&PHASE_9_DATA_RATE_BPS>0&&PHASE_9_PACKET_TTL_SECONDS>0);
    Phase10_Record_Check(r,"TEST 4 - Link_Stability remains routing-only",true);
    auto a=Generate_Shared_Contact_Uniforms(40000u,200);
    auto b=Generate_Shared_Contact_Uniforms(40000u,200);
    auto c=Generate_Shared_Contact_Uniforms(40001u,200);
    Phase10_Record_Check(r,"TEST 5 - fixed-seed contact stream reproducible",a==b);
    Phase10_Record_Check(r,"TEST 6 - changed seed changes contact stream",a!=c);
    std::cout<<"Phase 10 preflight certification: "<<(r.Overall_Pass?"PASS":"FAIL")<<"\n\n";
    return r;
}
// ============================================================
// MAIN
...
int main()
{
    ...
    std::cout
        << "PHASE 11 ROUTING-INTELLIGENCE DIAGNOSTIC\n";
    ...
    Phase10_Certification_Result Phase10_Result=Run_Phase10_Preflight_Certification();
    if(!Phase10_Result.Overall_Pass) return 1;
    ...
    // Post-pilot diagnostic only. These are the five pilot worlds in
    // which B3 selected Relay 0 instead of the direct route.
    const std::array<std::uint32_t, 5> Diagnostic_Seeds =
    {
        100012u,
        100026u,
        100082u,
    ...
    const int Number_Of_Debug_Trials =
        static_cast<int>(Diagnostic_Seeds.size());

    const std::uint32_t Base_Seed = Diagnostic_Seeds.front();
    ...
    const std::string Results_Folder =
        "./"
        "Monte_Carlo_Experiment 4/Phase_11_Routing_Diagnostic";
    ...
    const std::string Matching_File_Name =
        Results_Folder +
        "/Phase11_RQ4_Matching_Check.csv";
    ...
    const std::string Diagnostic_File_Name =
        Results_Folder +
        "/Phase11_Routing_Score_and_Counterfactual_Diagnostic.csv";
    ...
    {
        std::cerr << "ERROR: Could not create Phase 11 diagnostic CSV.\n";
        return 1;
    }
    ...
    Diagnostic_File
        << "Trial,Seed,Route_Index,Route_Name,Route_Available,"
        << "Contact_Available,Route_Contact_Probability,PAT_Cost_s,"
        << "PAT_Efficiency,Predicted_Optical_Quality,Link_Stability,"
        << "Propagation_Delay_s,Propagation_Efficiency,B3_Score,B4_Score,"
        << "B3_Selected,B4_Selected,Counterfactual_Delivered,"
        << "Counterfactual_Dropped,Counterfactual_Reliability,"
        << "Counterfactual_Throughput_bps,Counterfactual_Median_Latency_s,"
        << "Counterfactual_P95_Latency_s,Counterfactual_PAT_Burden_s\n";


    int Phase10_Expected_Packet_Rows=0,Phase10_Observed_Packet_Records=0;
    bool Phase10_All_Accounting_Valid=true;
    bool Phase10_All_Same_Route_Policies_Matched=true;
    bool Phase10_All_Metric_Recomputations_Matched=true;
         ...
         ++trial)
    {
        const std::uint32_t seed =
            Diagnostic_Seeds[static_cast<std::size_t>(trial - 1)];
        ...
        // Phase 11 diagnostic: expose every factor used by B3 and B4,
        // then force each feasible route through the same packet/contact
        // world. This measures the counterfactual result without changing
        // the frozen routing or packet-mechanics functions.
        const RQ4_Policy_Decision Phase11_B3_Decision =
            Evaluate_Policy(Routing_Policy::B3, Route_States);

        const RQ4_Policy_Decision Phase11_B4_Decision =
            Evaluate_Policy(Routing_Policy::B4, Route_States);

        const double Phase11_Minimum_PAT =
            Find_Minimum_Available_PAT_Cost(Route_States);

        const double Phase11_Minimum_Propagation =
            Find_Minimum_Available_Propagation_Delay(Route_States);

        for (std::size_t route_index = 0;
             route_index < Route_States.size();
             ++route_index)
        {
            const RQ4_Route_State& state = Route_States[route_index];

            const double pat_efficiency =
                state.PAT_Cost_Seconds > 0.0
                    ? Clamp_Zero_To_One(
                        Phase11_Minimum_PAT / state.PAT_Cost_Seconds)
                    : 0.0;

            const double propagation_efficiency =
                state.Propagation_Delay_Seconds > 0.0
                    ? Clamp_Zero_To_One(
                        Phase11_Minimum_Propagation /
                        state.Propagation_Delay_Seconds)
                    : 0.0;

            RQ4_Phase9_Trial_Metrics counterfactual_metrics{};

            if (state.Route.Available && state.Contact_Available)
            {
                RQ4_Policy_Decision forced_decision;
                forced_decision.Policy = Routing_Policy::B3;
                forced_decision.Selected_Route_Index =
                    static_cast<int>(route_index);
                forced_decision.Selected_Route_Name = state.Route.Route_Name;

                const std::vector<RQ4_Phase9_Packet_Record> forced_packets =
                ...
                counterfactual_metrics =
                    Calculate_Phase9_Trial_Metrics(forced_packets);
            }
            ...
            Diagnostic_File
                ...
                << (state.Route.Available ? 1 : 0) << ","
                << (state.Contact_Available ? 1 : 0) << ","
                << std::setprecision(17)
                << state.Route_Contact_Realization_Probability << ","
                << state.PAT_Cost_Seconds << ","
                << pat_efficiency << ","
                ...
                << state.Propagation_Delay_Seconds << ","
                << propagation_efficiency << ","
                << Phase11_B3_Decision.Route_Scores[route_index] << ","
                << Phase11_B4_Decision.Route_Scores[route_index] << ","
                << (Phase11_B3_Decision.Selected_Route_Index ==
                    static_cast<int>(route_index) ? 1 : 0) << ","
                << (Phase11_B4_Decision.Selected_Route_Index ==
                    static_cast<int>(route_index) ? 1 : 0) << ","
                << counterfactual_metrics.Delivered_Packets << ","
                << counterfactual_metrics.Dropped_Packets << ","
                << counterfactual_metrics.Reliability << ","
                << counterfactual_metrics.Delivered_Throughput_Bps << ","
                << counterfactual_metrics.Median_Delivered_Latency_Seconds << ","
                << counterfactual_metrics.P95_Delivered_Latency_Seconds << ","
                << counterfactual_metrics.Total_PAT_Burden_Seconds
                << "\n";
        }
        ...
        std::array<RQ4_Phase9_Trial_Metrics,4> Phase10_Metrics_By_Policy{};
        std::array<int,4> Phase10_Selected_Route_By_Policy{{-99,-99,-99,-99}};
        std::size_t Phase10_Policy_Slot=0;
             ...
             Policies)
        {
            ...
            const auto phase10_recomputed_metrics=Calculate_Phase9_Trial_Metrics(phase9_packets);
            if(!Phase10_Metrics_Equivalent(phase9_metrics,phase10_recomputed_metrics))
                Phase10_All_Metric_Recomputations_Matched=false;
            if(!Phase10_Packet_Accounting_Is_Valid(phase9_metrics))
                Phase10_All_Accounting_Valid=false;
            Phase10_Observed_Packet_Records+=static_cast<int>(phase9_packets.size());
            Phase10_Expected_Packet_Rows+=world.Generated_Packets;
            Phase10_Metrics_By_Policy[Phase10_Policy_Slot]=phase9_metrics;
            Phase10_Selected_Route_By_Policy[Phase10_Policy_Slot]=decision.Selected_Route_Index;
        ...
        for(std::size_t i=0;i<Policies.size();++i)
            for(std::size_t j=i+1;j<Policies.size();++j)
                if(Phase10_Selected_Route_By_Policy[i]==Phase10_Selected_Route_By_Policy[j] &&
                   !Phase10_Metrics_Equivalent(Phase10_Metrics_By_Policy[i],Phase10_Metrics_By_Policy[j]))
                    Phase10_All_Same_Route_Policies_Matched=false;
    }
    ...
    Summary_File
        ...
        << "Phase10_Preflight_Certification,Phase10_Packet_Accounting,"
        << "Phase10_Metric_Recomputation,Phase10_Same_Route_CRN_Matching,"
        << "Phase10_Expected_Packet_Rows,Phase10_Observed_Packet_Records,"
        << "Phase10_Validation_Seed_Range,Phase10_Pilot_Seed_First,"
        << "Phase10_Production_Seed_First,Phase10_Final_Readiness_Status\n";
    ...
    Summary_File
        ...
        << "NO,"
        << (Phase10_Result.Overall_Pass?"PASS":"FAIL") << ","
        << (Phase10_All_Accounting_Valid?"PASS":"FAIL") << ","
        << (Phase10_All_Metric_Recomputations_Matched?"PASS":"FAIL") << ","
        << (Phase10_All_Same_Route_Policies_Matched?"PASS":"FAIL") << ","
        << Phase10_Expected_Packet_Rows << "," << Phase10_Observed_Packet_Records << ","
        << "40000-49999," << PHASE_10_PILOT_SEED_FIRST << "," << PHASE_10_PRODUCTION_SEED_FIRST << ","
        << ((Phase10_Result.Overall_Pass&&Phase10_All_Accounting_Valid&&
             Phase10_All_Metric_Recomputations_Matched&&Phase10_All_Same_Route_Policies_Matched&&
             Phase10_Expected_Packet_Rows==Phase10_Observed_Packet_Records)?"PASS":"FAIL")
        << "\n";


    Phase10_Record_Check(Phase10_Result,"TEST 7 - packet outcome accounting identities",Phase10_All_Accounting_Valid);
    Phase10_Record_Check(Phase10_Result,"TEST 8 - trial metrics reproduce from packet records",Phase10_All_Metric_Recomputations_Matched);
    Phase10_Record_Check(Phase10_Result,"TEST 9 - same-route policies preserve matched CRN outcomes",Phase10_All_Same_Route_Policies_Matched);
    Phase10_Record_Check(Phase10_Result,"TEST 10 - packet audit row accounting",
                         Phase10_Expected_Packet_Rows==Phase10_Observed_Packet_Records);
    ...
    Phase10_Record_Check(Phase10_Result,"TEST 11 - trial output file exists and is nonempty",Phase10_File_Exists_And_Nonempty(Trial_File_Name));
    Phase10_Record_Check(Phase10_Result,"TEST 12 - summary output file exists and is nonempty",Phase10_File_Exists_And_Nonempty(Summary_File_Name));
    Phase10_Record_Check(Phase10_Result,"TEST 13 - matching diagnostic exists and is nonempty",Phase10_File_Exists_And_Nonempty(Matching_File_Name));
    Phase10_Record_Check(Phase10_Result,"TEST 14 - packet audit exists and is nonempty",Phase10_File_Exists_And_Nonempty(Packet_File_Name));
    Phase10_Record_Check(Phase10_Result,"TEST 15 - Phase 11 routing diagnostic exists and is nonempty",Phase10_File_Exists_And_Nonempty(Diagnostic_File_Name));
    ...
    std::cout
        << "Phase 11 routing diagnostic file:\n"
        << Diagnostic_File_Name
        << "\n\n";


    std::cout<<"For each Trial/Seed, B1-B4 retain identical shared-world, route-state, packet-release, and shared contact inputs.\n";
    std::cout<<"NOTE: This is a post-pilot diagnostic. Frozen Phase 10 physics and packet mechanics remain unchanged.\n";
    std::cout<<"\n====================================================\nPHASE 11 ROUTING-DIAGNOSTIC INTEGRITY CHECK\n====================================================\n"
             <<"Checks passed: "<<Phase10_Result.Checks_Passed<<" / "<<Phase10_Result.Checks_Total<<"\n"
             <<"Expected packet records: "<<Phase10_Expected_Packet_Rows<<"\n"
             <<"Observed packet records: "<<Phase10_Observed_Packet_Records<<"\n"
             <<"Pilot seed start: "<<PHASE_10_PILOT_SEED_FIRST<<"\n"
             <<"Production seed start: "<<PHASE_10_PRODUCTION_SEED_FIRST<<"\n"
             <<"PHASE 11 DIAGNOSTIC STATUS: "<<(Phase10_Result.Overall_Pass?"PASS":"FAIL")<<"\n";
    if(!Phase10_Result.Overall_Pass) return 1;
