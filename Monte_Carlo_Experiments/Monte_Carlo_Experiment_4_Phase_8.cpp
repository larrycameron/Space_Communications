...
// ============================================================
// PHASE 8 - PACKET DELIVERY MECHANICS AND END-TO-END OUTCOMES
//
// IMPORTANT SCOPE:
...
int main()
{
         ...
         ++trial)
    {
             ...
             Policies)
        {
            ...
            // Phase 8 evaluates one representative packet (packet index 0)
            // from the already-matched shared world. Full multi-packet trial
            // metrics are intentionally deferred to Phase 9.
            const RQ4_Packet_Outcome packet_outcome =
                Evaluate_Packet_Outcome(
                    0,
                    world.Packet_Release_Times_Seconds.front(),
                    PHASE_8_PACKET_SIZE_BITS,
                    PHASE_8_DATA_RATE_BPS,
                    PHASE_8_PACKET_TTL_SECONDS,
                    decision,
                    Route_States);
    ...
    Summary_File
        ...
        << "First_Seed,"
        << "Last_Seed,"
        << "Phase7_Policy_Self_Check,"
        << "Phase8_Packet_Mechanics_Self_Check,"
        << "Phase8_Packet_Size_bits,"
        << "Phase8_Data_Rate_bps,"
        << "Phase8_TTL_s,"
        << "Phase8_Packets_Evaluated_Per_Policy\n";
    ...
    std::cout
        << "Phase 8 packet outcomes may differ only because policies select "
        << "different routes.\n";

    std::cout
        << "NOTE: Phase 8 evaluates packet mechanics only; "
